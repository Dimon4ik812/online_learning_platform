...
from users.models import CustomsUser, Payments, Course
...
class PaymentsTests(APITestCase):
    ...
    def test_create_product(self):
        ...
        product = create_stripe_product(self.course)  # Используйте объект курса, а не строку
        self.assertIn("id", product)  # Проверяем, что продукт был создан и имеет ID

    def test_create_price(self):
        """Тестирование создания цены в Stripe"""
        product = create_stripe_product(self.course)  # Используйте объект курса, а не строку
        price = create_stripe_price(1000, product.id)  # 1000 рублей
