...
class PaymentsViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        course_id = request.data.get("course_id")  # ID курса, если нужно

        # Проверка на валидность входных данных
        if not product_name or amount is None:
            return Response(
                {"error": "Invalid input: product_name and amount are required"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            # Создаем продукт и получаем его ID
            product = create_stripe_product(course)

            # Создаем цену и получаем её ID
            price = create_stripe_price(amount, product.id)

            # Создаем сессию и получаем её ID
